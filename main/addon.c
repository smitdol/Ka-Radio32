...
char irStr[4];
xQueueHandle irQueue = NULL;
xQueueHandle lcdQueue = NULL;
...
IRAM_ATTR  void ServiceAddon(void)
{
	...
	if (timer1s >=1000)
	{
		...
		if (((timein % DTIDLE)==0)&&(!state)  ) {
			itAskStime=true;
			timein = 0; // start the time display when paused
	        }
...
{
// increment or decrement futureNum (aka next station) depending on sign of value (up/down)
	currentValue = value; // save sign (up/down)
	ESP_LOGD(TAG,"changeStation val: %d, futurnum: %d",value,futurNum);
	if (value < 0) futurNum--; else futurNum++;
	// check rollover
...
void drawStation()
{
	...
	do {
		...
		ddot = si->name;
		ptl = ddot;
		while ( *ptl == 0x20){ddot++;ptl++;} // skip blanks
		if (strlen(ddot)==0) {
			playable = false; // don't start an undefined station
			changeStation(currentValue);
		} else {
			playable = true;
		}
		free(si);
...
void updateStationOrVolume(bool role, int16_t newValue)
{
	if (newValue!=0)
	{
		typeScreen estate;
		if (role) estate = sstation; else estate = svolume;
		if (stateScreen != estate)
		{
...
void buttonUpdate(bool role, Button state1, Button state2)
{
	int16_t newValue = ((state1!=Open)?5:0)+((state2!=Open)?-5:0); // sstation take + or - in any value
	updateStationOrVolume(role, newValue);
...
void joystickCompute(Joystick_t *enc,bool role)
{
	...
	buttonUpdate(role, state1, state2);
}
...
void encoderLoop()
{
	...
	if (isEncoder0) encoderCompute(encoder0,SCTRL);
	if (isEncoder1) encoderCompute(encoder1,VCTRL);
}
...
void irLoop()
{
	// IR
	event_ir_t evt;
	while (xQueueReceive(irQueue, &evt, 0))
	{
		wakeLcd();
...
void task_lcd(void *pvParams)
{
	...
	while (1)
	{
		...
		if (timerScroll >= 500) //500 ms
		{
			...
			{
				mTscreen = MTREFRESH; // display time
			}
			drawScreen();
			timerScroll = 0;
		}
		while (xQueueReceive(lcdQueue, &evt, 0))
		{
			if (evt.lcmd != lmeta)
				ESP_LOGV(TAG,"lcdQueue: %x",(int)evt.lcmd);
			else
				ESP_LOGV(TAG,"lcdQueue: %x  %s",(int)evt.lcmd,evt.lline);
			switch(evt.lcmd)
			{
				case lmeta:
		 ...
		 if ((lcdQueue)&&(!uxQueueMessagesWaiting(lcdQueue))) vTaskDelay(4);
		vTaskDelay(1);
	}
	vTaskDelete( NULL );
}
...
void task_addon(void *pvParams)
{
	...
	irQueue = xQueueCreate(5, sizeof(event_ir_t));
	ESP_LOGD(TAG,"irQueue: %x",(int)irQueue);

	// queue for events of the lcd
	lcdQueue = xQueueCreate(20, sizeof(event_lcd_t));
	ESP_LOGD(TAG,"lcdQueue: %x",(int)lcdQueue);
...
//////Volume offset    ##CLI.OVOLSET#:
	...
	if (evt.lcmd != -1) xQueueSend(lcdQueue,&evt, 0);
	free (line);
}
